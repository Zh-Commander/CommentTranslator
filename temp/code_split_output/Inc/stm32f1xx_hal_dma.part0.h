...
/**
  * @brief  DMA 配置结构体定义
  *         用于配置 DMA 通道的基本参数，如传输方向、数据宽度、优先级等
  */
typedef struct
{
  uint32_t Direction;                 /*!< 指定数据传输方向：从内存到外设、内存到内存或外设到内存。
...
/**
  * @brief  HAL DMA 传输完成级别定义
  *         用于判断传输完成程度（半传输或全传输）
  */
typedef enum
{
  HAL_DMA_FULL_TRANSFER           = 0x00U,    /*!< 全传输完成     */
...
typedef enum
{
  HAL_DMA_XFER_CPLT_CB_ID          = 0x00U,    /*!< 全传输完成回调     */
...
/** 
  * @brief  DMA 句柄结构体定义
  *         DMA 操作的核心结构体，包含了 DMA 的配置、状态、回调函数等信息
  */
typedef struct __DMA_HandleTypeDef
{
  DMA_Channel_TypeDef        *Instance;                                                    /*!< 寄存器基地址，指向具体的 DMA 通道                  */
  
  DMA_InitTypeDef            Init;                                                         /*!< DMA 通信参数，初始化配置结构体           */
  
  HAL_LockTypeDef            Lock;                                                         /*!< DMA 锁对象，用于资源管理                     */
  
  __IO HAL_DMA_StateTypeDef  State;                                                        /*!< DMA 传输状态                     */
  
  void                       *Parent;                                                      /*!< 父对象状态，通常指向上层外设句柄（如 UART 句柄）                    */
  ...
  uint32_t                   ChannelIndex;                                                /*!< DMA 通道索引，用于计算标志位位置                      */
...
// 数字后缀对应通道号 (1-7)
#define DMA_FLAG_GL1                      0x00000001U // 通道1 全局标志
#define DMA_FLAG_TC1                      0x00000002U // 通道1 传输完成标志
#define DMA_FLAG_HT1                      0x00000004U // 通道1 半传输标志
#define DMA_FLAG_TE1                      0x00000008U // 通道1 传输错误标志
#define DMA_FLAG_GL2                      0x00000010U // 通道2 全局标志
#define DMA_FLAG_TC2                      0x00000020U // 通道2 传输完成标志
#define DMA_FLAG_HT2                      0x00000040U // 通道2 半传输标志
#define DMA_FLAG_TE2                      0x00000080U // 通道2 传输错误标志
#define DMA_FLAG_GL3                      0x00000100U // 通道3 全局标志
#define DMA_FLAG_TC3                      0x00000200U // 通道3 传输完成标志
#define DMA_FLAG_HT3                      0x00000400U // 通道3 半传输标志
#define DMA_FLAG_TE3                      0x00000800U // 通道3 传输错误标志
#define DMA_FLAG_GL4                      0x00001000U // 通道4 全局标志
#define DMA_FLAG_TC4                      0x00002000U // 通道4 传输完成标志
#define DMA_FLAG_HT4                      0x00004000U // 通道4 半传输标志
#define DMA_FLAG_TE4                      0x00008000U // 通道4 传输错误标志
#define DMA_FLAG_GL5                      0x00010000U // 通道5 全局标志
#define DMA_FLAG_TC5                      0x00020000U // 通道5 传输完成标志
#define DMA_FLAG_HT5                      0x00040000U // 通道5 半传输标志
#define DMA_FLAG_TE5                      0x00080000U // 通道5 传输错误标志
#define DMA_FLAG_GL6                      0x00100000U // 通道6 全局标志
#define DMA_FLAG_TC6                      0x00200000U // 通道6 传输完成标志
#define DMA_FLAG_HT6                      0x00400000U // 通道6 半传输标志
#define DMA_FLAG_TE6                      0x00800000U // 通道6 传输错误标志
#define DMA_FLAG_GL7                      0x01000000U // 通道7 全局标志
#define DMA_FLAG_TC7                      0x02000000U // 通道7 传输完成标志
#define DMA_FLAG_HT7                      0x04000000U // 通道7 半传输标志
#define DMA_FLAG_TE7                      0x08000000U // 通道7 传输错误标志
/**
  * @}
  */
