...
/* ADC双模式使能：组合规则同步 + 注入同步模式，作用于规则组和注入组 */
...
/* ADC双模式使能：组合规则同步 + 交替触发模式，作用于规则组和注入组 */
...
/* ADC双模式使能：组合注入同步 + 快速交错模式，作用于规则组和注入组（ADC采样阶段之间的延迟：7个ADC时钟周期） */
...
/* ADC双模式使能：组合注入同步 + 慢速交错模式，作用于规则组和注入组（ADC采样阶段之间的延迟：14个ADC时钟周期） */
...
/* ADC双模式使能：快速交错模式，作用于规则组（ADC采样阶段之间的延迟：7个ADC时钟周期） */
...
/* ADC双模式使能：慢速交错模式，作用于规则组（ADC采样阶段之间的延迟：14个ADC时钟周期） */
...
/* ADC1, ADC2, ADC3 (如果选定设备上有的话) 规则组的外部触发源列表。             */
/* (used internally by HAL driver. To not use into HAL structure parameters)  */
/* (由HAL驱动内部使用。不要在HAL结构体参数中使用)                              */

/* External triggers of regular group for ADC1&ADC2 (if ADCx available) */
/* ADC1和ADC2规则组的外部触发源 (如果ADCx可用) */
...
/* ADC1、ADC2、ADC3 规则组的外部触发源 (如果ADCx可用) */
...
/* ADC1、ADC2、ADC3 注入组的外部触发源 (如果ADCx可用) */
...
/**
  ...
  *        对于有3个ADC的设备：根据ADC属于公共组ADC1&ADC2还是ADC3，定义规则组的外部触发源
  *        （某些具有相同源的触发器在写入CR2寄存器的EXTSEL位时具有不同的值）。
  *        对于有2个或更少ADC的设备：此宏不做任何更改。
  * @param __HANDLE__: ADC handle / ADC句柄
  * @param __EXT_TRIG_CONV__: External trigger selected for regular group. / 为规则组选择的外部触发源。
  * @retval External trigger to be programmed into EXTSEL bits of CR2 register / 将被编程到CR2寄存器EXTSEL位的外部触发值
  */
#if defined (STM32F103xE) || defined (STM32F103xG)
...
/**
  ...
  *        For devices with 2 ADCs or less: this macro makes no change.
  *        对于有3个ADC的设备：根据ADC属于公共组ADC1&ADC2还是ADC3，定义注入组的外部触发源
  *        （某些具有相同源的触发器在写入CR2寄存器的JEXTSEL位时具有不同的值）。
  *        对于有2个或更少ADC的设备：此宏不做任何更改。
  * @param __HANDLE__: ADC handle / ADC句柄
  * @param __EXT_TRIG_INJECTCONV__: External trigger selected for injected group. / 为注入组选择的外部触发源。
  * @retval External trigger to be programmed into JEXTSEL bits of CR2 register / 将被编程到CR2寄存器JEXTSEL位的外部触发值
  */
#if defined (STM32F103xE) || defined (STM32F103xG)
...
/**
  ...
  *        验证所选ADC（多模式ADC主设备或ADC从设备）是否启用了多模式（适用于具有多个ADC的设备）
  * @param __HANDLE__: ADC handle / ADC句柄
  * @retval Multimode state: RESET if multimode is disabled, other value if multimode is enabled / 多模式状态：如果禁用多模式则为RESET，如果使能多模式则为其他值
  */
#if defined (STM32F103x6) || defined (STM32F103xB) || defined (STM32F105xC) || defined (STM32F107xC) || defined (STM32F103xE) || defined (STM32F103xG)
...
/**
  ...
  *        验证ADC启动转换的条件：ADC必须处于非多模式，或者处于具有ADC主设备句柄的多模式（适用于具有多个ADC的设备）
  * @param __HANDLE__: ADC handle / ADC句柄
  * @retval None / 无
  */
#if defined (STM32F103x6) || defined (STM32F103xB) || defined (STM32F105xC) || defined (STM32F107xC) || defined (STM32F103xE) || defined (STM32F103xG)
...
/**
  ...
  *        检查ADC多模式设置：在多模式情况下，检查所选ADC的ADC主设备是否启用了自动注入功能（适用于具有多个ADC的设备）
  * @param __HANDLE__: ADC handle / ADC句柄
  * @retval None / 无
  */
#if defined (STM32F103x6) || defined (STM32F103xB) || defined (STM32F105xC) || defined (STM32F107xC) || defined (STM32F103xE) || defined (STM32F103xG)
...
/**
  ...
  *        设置共享公共多模式设置的另一个ADC的句柄
  * @param __HANDLE__: ADC handle / ADC句柄
  * @param __HANDLE_OTHER_ADC__: other ADC handle / 另一个ADC句柄
  * @retval None / 无
  */
...
/**
  ...
  *        设置与ADC主设备关联的ADC从设备的句柄
  * On STM32F1 devices, ADC slave is always ADC2 (this can be different
  * on other STM32 devices)
  * 在STM32F1设备上，ADC从设备始终是ADC2（在其他STM32设备上可能不同）
  * @param __HANDLE_MASTER__: ADC master handle / ADC主设备句柄
  * @param __HANDLE_SLAVE__: ADC slave handle / ADC从设备句柄
  * @retval None / 无
  */
