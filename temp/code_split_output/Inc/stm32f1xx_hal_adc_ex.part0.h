...
typedef struct 
{
  uint32_t InjectedChannel;                       /*!< 要配置的ADC通道选择
                                                       ...
                                                       注意：根据设备不同，某些通道可能在封装引脚上不可用。请参考设备数据手册以了解通道可用性。
  ...
  uint32_t InjectedRank;                          /*!< 注入组序列器中的排名
                                                       ...
                                                       注意：如果需要禁用通道或更改转换序列的顺序，可以通过新的通道设置覆盖包含先前通道设置的排名 (或者调整参数转换次数) */
  ...
  uint32_t InjectedNbrOfConversion;               /*!< 指定注入组序列器中转换的排名数量。
  ...
  FunctionalState InjectedDiscontinuousConvMode;  /*!< 指定注入组的转换序列是执行完整序列还是不连续序列 (主序列细分为连续部分)。
                                                       只有在序列器使能 (参数 'ScanConvMode') 时才使用不连续模式。如果序列器禁用，则忽略此参数。
                                                       只有在连续模式禁用时才能使能不连续模式。如果使能连续模式，则忽略此参数设置。
                                                       此参数可设置为 ENABLE 或 DISABLE。
                                                       注意：对于注入组，不连续排名增量固定为逐个增加。
                                                       注意：此设置影响整个注入组。因此，调用 HAL_ADCEx_InjectedConfigChannel() 
                                                             配置注入组上的通道可能会影响先前设置的其他通道的配置。 */
  ...
  uint32_t ExternalTrigInjecConv;                 /*!< 选择用于触发注入组开始转换的外部事件。
                                                       ...
                                                       注意：必须在ADC禁用时修改此参数 (在ADC开始转换之前或ADC停止转换之后)。
                                                             如果ADC使能，此参数设置将被绕过且不报告错误 (因为在动态更新其他参数时可能是预期行为)
                                                       注意：此设置影响整个注入组。因此，调用 HAL_ADCEx_InjectedConfigChannel() 
                                                             配置注入组上的通道可能会影响先前设置的其他通道的配置。 */
...
typedef struct
{
  uint32_t Mode;              /*!< 配置ADC在独立模式或多模式下工作。 
                                   ...
                                   注意：在使用同步模式的情况下：应为ACD1和ADC2同时采样的2个通道配置完全相同的采样时间。
                                   注意：在使用交错模式的情况下：为避免转换重叠，快速交错模式允许的最大采样时间为7个ADC时钟周期，慢速交错模式为14个ADC时钟周期。
                                   注意：某些多模式参数在STM32F1上是固定的，而在具有多个ADC的其他STM32设备上可配置 (多模式配置结构体可能有额外参数)。
                                         等效关系如下：
                                           - 参数 'DMAAccessMode'：在STM32F1上，此参数固定为1个DMA通道 (两个ADC共用一个DMA通道，即ADC主设备的DMA)。在具有多个ADC的其他STM32设备上，这等效于参数 'ADC_DMAACCESSMODE_12_10_BITS'。
                                           - 参数 'TwoSamplingDelay'：在STM32F1上，根据选择的快速或慢速交错模式，此参数固定为7或14个ADC时钟周期。在具有多个ADC的其他STM32设备上，这等效于参数 'ADC_TWOSAMPLINGDELAY_7CYCLES' (用于快速交错模式)。 */
...
/** @defgroup ADCEx_External_trigger_edge_Injected ADCEx 注入组外部触发使能
  * @{
  */
#define ADC_EXTERNALTRIGINJECCONV_EDGE_NONE           0x00000000U  // 禁用外部触发 (软件触发)
#define ADC_EXTERNALTRIGINJECCONV_EDGE_RISING         ((uint32_t)ADC_CR2_JEXTTRIG)  // 外部触发上升沿使能
/**
  * @}
  */
    
/** @defgroup ADC_External_trigger_source_Regular ADC 常规组外部触发源选择
  * @{
  */
...
/*         要在 ADC 或 ADC2 上使用它，必须使用宏从 EXTI 线 11 重映射触发：     */
/*           __HAL_AFIO_REMAP_ADC1_ETRGREG_ENABLE()                           */
/*           __HAL_AFIO_REMAP_ADC2_ETRGREG_ENABLE()                           */

/* 内部常量值管理注意：如果 TIM8_TRGO 可用，    */
/* 其定义默认设置为 ADC1&ADC2 的值，如果选择了 ADC3，则由 HAL ADC 驱动程序更改为 ADC3 的值。                      */
...
/** @defgroup ADCEx_External_trigger_source_Injected ADCEx 注入组外部触发源选择
  * @{
  */
/*!< 具有通用触发名称的外部触发列表，独立于ADC目标，按触发名称排序： */
...
/*         要在 ADC1 或 ADC2 上使用它，必须使用宏从 EXTI 线 11 重映射触发：                               */
