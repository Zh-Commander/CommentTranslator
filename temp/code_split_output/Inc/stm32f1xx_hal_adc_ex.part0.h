...
/** 
  ...
  * @note   使用函数 HAL_ADCEx_InjectedConfigChannel() 设置这些参数的前提是 ADC 处于特定状态。
  *         ADC 状态可以是：
  *          - 对于所有参数：ADC 处于禁用状态 (这是修改参数 'ExternalTrigInjecConv' 唯一可能的状态)
  *          - 对于除 'ExternalTrigInjecConv' 之外的所有参数：ADC 处于使能状态且注入组没有正在进行的转换。
  */
typedef struct 
{
  uint32_t InjectedChannel;                       /*!< 选择要配置的 ADC 通道
                                                       ...
                                                       注意：在具有多个 ADC 的 STM32F1 设备上：只有 ADC1 可以访问内部测量通道 (VrefInt/TempSensor)
                                                       注意：在 STM32F10xx8 和 STM32F10xxB 设备上：当 ADC 使用注入触发进行转换时，可能会在 PA0 引脚 (ADC 输入 0) 上产生低幅度电压毛刺。
                                                             建议分配模拟通道时，将通道 0 配置为注入通道。
                                                             更多详情请参考这些设备的勘误手册。 */
  ...
  FunctionalState InjectedDiscontinuousConvMode;  /*!< 指定注入组的转换序列是执行完整序列还是不连续序列 (主序列细分为连续部分)。
                                                       ...
                                                       注意：此设置会影响整个注入组。因此，调用 HAL_ADCEx_InjectedConfigChannel() 
                                                             配置注入组上的通道可能会影响先前设置的其他通道的配置。 */
  FunctionalState AutoInjectedConv;               /*!< 使能或禁用所选 ADC 在常规组转换后自动进行注入组转换
                                                       此参数可设置为 ENABLE 或 DISABLE。
                                                       ...
                                                       注意：如果常规组使用 DMA：如果 DMA 配置为正常模式 (单次)，则 JAUTO 将在 DMA 传输完成后停止。
  ...
  uint32_t ExternalTrigInjecConv;                 /*!< 选择用于触发注入组开始转换的外部事件。
                                                       ...
                                                       注意：必须在 ADC 禁用时修改此参数 (在 ADC 开始转换之前或 ADC 停止转换之后)。
                                                             如果 ADC 已启用，此参数设置将被绕过且不报告错误 (因为在动态更新其他参数时，这可能是预期行为)
                                                       注意：此设置会影响整个注入组。因此，调用 HAL_ADCEx_InjectedConfigChannel() 
                                                             配置注入组上的通道可能会影响先前设置的其他通道的配置。 */
...
/** 
  * @brief  ADC 多模式结构体定义
  * @note   使用函数 HAL_ADCEx_MultiModeConfigChannel() 设置这些参数的前提是 ADC 状态 (公共组的两个 ADC)。
  *         公共组的 ADC 状态必须为：禁用。
  */
typedef struct
{
  uint32_t Mode;              /*!< 配置 ADC 在独立模式或多模式下工作。 
                                   ...
                                   注意：在双模式下，更改通道配置会产生重启，可能导致同步丢失。建议在任何配置更改之前禁用双模式。
                                   注意：在使用同时模式的情况下：应为 ADC1 和 ADC2 同时采样的两个通道配置完全相同的采样时间。
                                   注意：在使用交错模式的情况下：为避免转换重叠，快速交错模式允许的最大采样时间为 7 个 ADC 时钟周期，慢速交错模式为 14 个 ADC 时钟周期。
                                   注意：某些多模式参数在 STM32F1 上是固定的，而在具有多个 ADC 的其他 STM32 设备上可以配置 (多模式配置结构体可能有额外的参数)。
                                         等效关系如下：
                                           - 参数 'DMAAccessMode'：在 STM32F1 上，此参数固定为 1 个 DMA 通道 (两个 ADC 共用一个 DMA 通道，即 ADC 主机的 DMA)。在具有多个 ADC 的其他 STM32 设备上，这等效于参数 'ADC_DMAACCESSMODE_12_10_BITS'。
...
/** @defgroup ADCEx_injected_rank ADCEx rank into injected group
  * @{
  */
#define ADC_INJECTED_RANK_1                           0x00000001U // 注入组排名 1
#define ADC_INJECTED_RANK_2                           0x00000002U // 注入组排名 2
#define ADC_INJECTED_RANK_3                           0x00000003U // 注入组排名 3
#define ADC_INJECTED_RANK_4                           0x00000004U // 注入组排名 4
/**
  * @}
  */
...
/** @defgroup ADCEx_External_trigger_edge_Injected ADCEx external trigger enable for injected group
  * @{
  */
#define ADC_EXTERNALTRIGINJECCONV_EDGE_NONE           0x00000000U // 注入组外部触发禁用
#define ADC_EXTERNALTRIGINJECCONV_EDGE_RISING         ((uint32_t)ADC_CR2_JEXTTRIG) // 注入组外部触发上升沿使能
/**
  * @}
  */
