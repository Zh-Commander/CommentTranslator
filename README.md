...
翻译完成后，建议使用 Beyond Compare（BCompare）等文本对比工具，对原始源码和输出源码进行人工检查。

重点检查：

- 是否误改了代码、标识符、宏定义或预处理结构。
- 是否出现多余的代码块标记，例如 ```` ```c ```` 或 ```` ``` ````。
- 分块边界附近是否被 AI 补全了不属于当前分块的代码，例如额外 `#endif`。
- 宏续行末尾反斜杠 `\` 是否仍保持合法位置。
- 新增中文注释是否准确，是否存在明显误导。

### 模型选择建议

建议使用 GLM-5、Kimi K2.6，或编码能力相近、更强的模型。较强的代码理解模型通常能获得更精确、更丰富的注释翻译和补充，同时显著减少人工修补工作量。

笔者测试项目：STM32G474 HAL 库。

| 模型 | 人工修补情况 |
|---|---|
| GLM-5 | 1 处人工修补，位于 `stm32_hal_legacy.h` |
| Kimi K2.6 | 4 处人工修补，位于 `stm32_hal_legacy.h` |

以上结果只代表当前 Prompt、当前配置和该测试项目下的经验，后续更换模型、Prompt、切分参数或源码类型后，应重新评估输出质量。
