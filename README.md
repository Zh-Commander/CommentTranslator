...
各位在学习单片机时是否为函数名宏定义变量名全是缩写又不写注释的STM32库而烦恼，是否被一坨一坨的英文注释困扰，于是我想到似乎可以用AI将整个HAL库LL库的英文注释翻译一遍，并且为每个宏定义和函数，关键代码处添加详尽的中文注释。于是，就有了这个项目。



目前项目是为C/C++设计的，当然你也可以改改，将这个项目用于处理其他语言的代码。
...
目前仅支持OpenAI类型接口，后续考虑其他接口

建议使用NewAPI将其他类型接口中转成OpenAI类型接口
...
- 代码量大的场景建议买一个支持高并发的coding plan，否则花销将是天文数字
