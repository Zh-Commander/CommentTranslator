/**
  ...
  * 保留所有权利。
  *
  * 本软件根据在本软件根目录的LICENSE文件中可以找到的条款进行许可。
...
typedef struct
{
  uint32_t Direction;                 /*!< 指定数据传输方向：从外设到内存、从内存到外设或从内存到内存。
                                           该参数可以是 @ref DMA_Data_transfer_direction 中的一个值 */

  uint32_t PeriphInc;                 /*!< 指定外设地址寄存器是否自增。
                                           该参数可以是 @ref DMA_Peripheral_incremented_mode 中的一个值 */

  uint32_t MemInc;                    /*!< 指定内存地址寄存器是否自增。
                                           该参数可以是 @ref DMA_Memory_incremented_mode 中的一个值 */

  uint32_t PeriphDataAlignment;       /*!< 指定外设数据宽度（字节/半字/字）。
                                           该参数可以是 @ref DMA_Peripheral_data_size 中的一个值 */

  uint32_t MemDataAlignment;          /*!< 指定内存数据宽度（字节/半字/字）。
                                           该参数可以是 @ref DMA_Memory_data_size 中的一个值 */

  uint32_t Mode;                      /*!< 指定 DMAy Channelx 的操作模式（正常模式或循环模式）。
                                           该参数可以是 @ref DMA_mode 中的一个值
                                           @note 如果在选定的通道上配置了内存到内存的数据传输，
                                                 则不能使用循环缓冲模式 */

  uint32_t Priority;                  /*!< 指定 DMAy Channelx 的软件优先级。
                                           该参数可以是 @ref DMA_Priority_level 中的一个值 */
} DMA_InitTypeDef;
...
typedef enum
{
  ...
  HAL_DMA_STATE_READY             = 0x01U,  /*!< DMA 已初始化并准备就绪      */
  HAL_DMA_STATE_BUSY              = 0x02U,  /*!< DMA 正在处理传输                 */
  HAL_DMA_STATE_TIMEOUT           = 0x03U   /*!< DMA 超时状态                      */
}HAL_DMA_StateTypeDef;

/**
  * @brief  HAL DMA 完成级别结构体定义（用于传输完成判断）
  */
typedef enum
{
...
/** @defgroup DMA_Error_Code DMA 错误代码
  * @{
  */
...
/** @defgroup DMA_Peripheral_incremented_mode DMA 外设地址自增模式
  * @{
  */
...
/** @defgroup DMA_Memory_incremented_mode DMA 内存地址自增模式
  * @{
  */
...
/** @defgroup DMA_Peripheral_data_size DMA 外设数据宽度
  * @{
  */
...
/** @defgroup DMA_Memory_data_size DMA 内存数据宽度
  * @{
  */
...
/** @defgroup DMA_interrupt_enable_definitions DMA 中断使能定义
  * @{
  */
#define DMA_IT_TC                         ((uint32_t)DMA_CCR_TCIE) // 传输完成中断使能
#define DMA_IT_HT                         ((uint32_t)DMA_CCR_HTIE) // 半传输完成中断使能
#define DMA_IT_TE                         ((uint32_t)DMA_CCR_TEIE) // 传输错误中断使能
/**
  * @}
  */

/** @defgroup DMA_flag_definitions DMA 标志定义
  * @{
  */
// 以下定义了DMA通道1的标志位
#define DMA_FLAG_GL1                      0x00000001U // 全局标志位
#define DMA_FLAG_TC1                      0x00000002U // 传输完成标志位
#define DMA_FLAG_HT1                      0x00000004U // 半传输完成标志位
#define DMA_FLAG_TE1                      0x00000008U // 传输错误标志位

// 以下定义了DMA通道2的标志位
#define DMA_FLAG_GL2                      0x00000010U // 全局标志位
#define DMA_FLAG_TC2                      0x00000020U // 传输完成标志位
#define DMA_FLAG_HT2                      0x00000040U // 半传输完成标志位
#define DMA_FLAG_TE2                      0x00000080U // 传输错误标志位

// 以下定义了DMA通道3的标志位
#define DMA_FLAG_GL3                      0x00000100U // 全局标志位
#define DMA_FLAG_TC3                      0x00000200U // 传输完成标志位
#define DMA_FLAG_HT3                      0x00000400U // 半传输完成标志位
#define DMA_FLAG_TE3                      0x00000800U // 传输错误标志位

// 以下定义了DMA通道4的标志位
#define DMA_FLAG_GL4                      0x00001000U // 全局标志位
#define DMA_FLAG_TC4                      0x00002000U // 传输完成标志位
#define DMA_FLAG_HT4                      0x00004000U // 半传输完成标志位
#define DMA_FLAG_TE4                      0x00008000U // 传输错误标志位

// 以下定义了DMA通道5的标志位
#define DMA_FLAG_GL5                      0x00010000U // 全局标志位
#define DMA_FLAG_TC5                      0x00020000U // 传输完成标志位
#define DMA_FLAG_HT5                      0x00040000U // 半传输完成标志位
#define DMA_FLAG_TE5                      0x00080000U // 传输错误标志位

// 以下定义了DMA通道6的标志位
#define DMA_FLAG_GL6                      0x00100000U // 全局标志位
#define DMA_FLAG_TC6                      0x00200000U // 传输完成标志位
#define DMA_FLAG_HT6                      0x00400000U // 半传输完成标志位
#define DMA_FLAG_TE6                      0x00800000U // 传输错误标志位

// 以下定义了DMA通道7的标志位
#define DMA_FLAG_GL7                      0x01000000U // 全局标志位
#define DMA_FLAG_TC7                      0x02000000U // 传输完成标志位
#define DMA_FLAG_HT7                      0x04000000U // 半传输完成标志位
#define DMA_FLAG_TE7                      0x08000000U // 传输错误标志位
/**
  * @}
  */
